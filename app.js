...
const head = document.querySelector('#rankingsHead');
...
function render(){
  ...
  const scored = teams.map(t => ({...t, score: categories.reduce((sum,c)=>sum + norm[c.key] * ((33-t[c.key])/32) * 100,0)}));
  const mean = scored.reduce((sum,t)=>sum+t.score,0)/scored.length;
  const sd = Math.sqrt(scored.reduce((sum,t)=>sum+Math.pow(t.score-mean,2),0)/scored.length);
  const sorted = scored.map(t=>({...t, expectedWins:8.5 + 2.15 * ((t.score-mean)/sd)})).sort((a,b)=>b.score-a.score || a.name.localeCompare(b.name));
  head.innerHTML = activeView === 'rankings'
    ? '<tr><th class="rank-col">RK</th><th>TEAM</th><th class="score-col">INDEX</th><th>QB</th><th>DEF</th><th>COACH</th><th>OL</th><th>SKILL</th></tr>'
    : '<tr><th class="rank-col">RK</th><th>TEAM</th><th>EXPECTED RECORD</th><th>WINS</th><th>LOSSES</th><th class="score-col">INDEX</th></tr>';
  body.innerHTML = sorted.map((t,i) => ({...t, overall:i+1})).filter(t=>t.name.toLowerCase().includes(query)).map(t => {
    ...
    const teamCell = `<td class="team-cell"><div class="team-wrap"><span class="team-badge" style="--team:${color}">${abbr}</span><span class="team-name">${t.name}<small class="conf">${conf}</small></span></div></td>`;
    const scoreCell = `<td><div class="score-wrap"><span class="score-bar"><i style="--score:${t.score}%;--tone:${tone}"></i></span><span class="score" style="--tone:${tone}">${t.score.toFixed(1)}</span></div></td>`;
    if(activeView === 'wins'){
      const wins=t.expectedWins.toFixed(1), losses=(17-t.expectedWins).toFixed(1);
      return `<tr><td class="rank-num">${t.overall}</td>${teamCell}<td><span class="record">${wins}–${losses}</span></td><td class="wins-value">${wins}</td><td class="losses-value">${losses}</td>${scoreCell}</tr>`;
    }
    return `<tr><td class="rank-num">${t.overall}</td>${teamCell}${scoreCell}${categories.map(c=>`<td><span class="rank-pill">${t[c.key]}</span></td>`).join('')}</tr>`;
  }).join('') || `<tr><td colspan="${activeView==='wins'?6:8}" style="text-align:center;height:100px">No teams found.</td></tr>`;
}
...
document.querySelectorAll('.table-tab').forEach(tab=>tab.addEventListener('click',()=>{
  ...
  document.querySelectorAll('.table-tab').forEach(t=>{const selected=t===tab;t.classList.toggle('active',selected);t.setAttribute('aria-selected',selected)});
  document.querySelector('#projectionNote').hidden=activeView!=='wins';
  render();
}));
buildSliders(); render();
